...
public class MembreFacade extends AbstractFacade<Membre> implements MembreFacadeLocal {
    ...
    @Override
    public List<Membre> find_by_commentaire(Erreur e) {
        Query q = em.createQuery("Select DISTINCT m.nom from membre m, commentaire c where m.idmembre = c.idmembre and c.iderreur= :iderreur");
        q.setParameter("iderreur", e.getIderreur());
        try {
